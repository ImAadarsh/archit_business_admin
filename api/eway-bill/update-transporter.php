...
$ebn = (string) ($in['ebn'] ?? '');
$tid = (string) ($in['transporterId'] ?? '');

if ($business_id <= 0 || $ebn === '' || $tid === '') {
    echo json_encode(['status' => 'error', 'message' => 'business_id, ebn and transporterId are required.']);
    exit;
}
...
echo json_encode($controller->updateEwayBillTransporter($business_id, $ebnClean, $tid), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
