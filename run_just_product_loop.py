#!/usr/bin/env python3
"""
Hit just-product generator one product at a time.
Continues after success or IMAGE_SAFETY skip.
Stops on other generation failures, HTTP errors, or empty queue.

Usage:
  python3 run_just_product_loop.py
  python3 run_just_product_loop.py --max 50
  python3 run_just_product_loop.py --url 'https://dashboard.invoicemate.in/generate_just_product_images.php?GEMINI_API_KEY=...'
"""
...
BASE_URL = "https://dashboard.invoicemate.in/generate_just_product_images.php"
...
def default_url() -> str:
    ...
    params = urllib.parse.urlencode(
        {"GEMINI_API_KEY": key, "IMAGE_MODEL": DEFAULT_IMAGE_MODEL}
    )
...
def fetch(url: str, timeout: int) -> tuple[int, str]:
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "just-product-loop/1.0"},
        method="GET",
    )
...
def classify(body: str) -> tuple[str, str]:
    ...
    if "Fatal error" in body or "Uncaught" in body:
        return "stop_fail", "PHP fatal error"

    status_m = STATUS_RE.search(body)
    status = status_m.group(1).strip() if status_m else ""
    status_plain = re.sub(r"<[^>]+>", "", status).strip()

    if "Marked as processed" in status_plain or "Status: Processed" in body:
        return "continue", status_plain or "processed"
    ...
    if "Image Uploaded: Yes" in body or "Upload successful!" in body:
        return "continue", "uploaded"
    if "rate-limited (HTTP 429)" in body.lower() or "http_429" in body.lower():
        return "stop_fail", "rate limited (429)"
    if "Failed to generate image" in body and "IMAGE_SAFETY" not in body:
        return "stop_fail", "generation failed"

    return "stop_fail", status_plain or "unrecognized response / failed run"


def main() -> int:
    parser = argparse.ArgumentParser(description="Sequential just-product generator runner")
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="HTTP timeout seconds per product (OpenAI fallback can be slow)",
    )
